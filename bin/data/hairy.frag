...
vec2 map( vec2 p, in float offset )
{
    ...
    float a = noise(p*1.5 + sin(0.1*u_time))*6.2831;
    a -= offset;
...
void main()
{
    ...
    float offset = u_time + gl_FragCoord.x/u_resolution.x;
    ...
    for( int i=0; i<32; i++ )
    {
        ...
        vec3 ttt = w*texture2D( tex0, uv ).xyz;
    ...
    col += vec3(0.4)*dot( nor, vec3(0.7,0.71,0.7) );
